...
var id3Async = Promise.promisify(id3);  // TODO: Might be used later
...
var PodcastServer = function () {
    ...
    var getMediaType = function (filename) {
        if (_.includes(options.videoExtensions, path.extname(filename))) {
            return 'video';
        } else if (_.includes(options.audioExtensions, path.extname(filename))) {
            return 'audio';
        } else if (_.includes(options.otherExtensions, path.extname(filename))) {
            return 'other';
        } else {
            return 'none';
        }
    ...
    var isCoverArt = function (filename) {
        return _.includes(options.coverArtFiles.map(function (covername) {
    ...
    var getFeedPath = function (path) {
        ...
        return getSubDirs(options.documentRoot)
            ...
            .then(function renderIndexTemplate(feeds) {
                titleSearch = _.filter(feeds, { 'title': path });
                if (titleSearch.length > 0) {
                    return titleSearch[0].title;
                }
                hashSearch = _.filter(feeds, { 'hash': path });
